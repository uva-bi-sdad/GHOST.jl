...
function setup(dbname::AbstractString = "sdad", schema::AbstractString = "gh_2007_$(year(floor(now(utc_tz), Year) - Day(1)))")
    ...
    execute(conn,
            replace(join(["CREATE EXTENSION IF NOT EXISTS btree_gist; CREATE SCHEMA IF NOT EXISTS schema;",
                          String(read(joinpath("src", "assets", "sql", "licenses.sql"))),
                          String(read(joinpath("src", "assets", "sql", "queries.sql"))),
                          String(read(joinpath("src", "assets", "sql", "repos.sql"))),
                          String(read(joinpath("src", "assets", "sql", "commits.sql"))),
                         ],
                        ' '),
                    "schema" => schema))
    nothing
